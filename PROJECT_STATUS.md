...
rows freed by finished sequences are refilled at the next decode step, with
deadline-aware admission and a drain on shutdown.
...
| Python test suite | **570 tests — pass**, 95% statement / 85% branch coverage |
...
| Prefill cost vs batch width | A 6-token prompt costs **7.9× less** to prefill in a batch of 32 than alone (1.863 → 0.235 ms/prompt). What sets the cost is the width each prefill achieves, not the number of prefills — which is why continuous batching's prefill is dearer at width ~4 than static's at ~32 |
